      ...
      {users
        ...
        .map((user) => {
          const remoteStream = remoteStreams.find(
            (rs) => rs.userId === user.userId
          );
          const isRemoteVideoEnabled = remoteStream?.isVideoEnabled ?? false;
          const isRemoteAudioEnabled = remoteStream?.isAudioEnabled ?? false;

          return (
            <Card
              key={user.userId}
              className="relative overflow-hidden aspect-video bg-slate-800"
            >
              <div className="w-full h-full flex items-center justify-center">
              ...
              <div className="absolute bottom-2 left-2 flex gap-1 z-10">
                <div
                  className={`p-1 rounded-full ${
                    isRemoteAudioEnabled ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                ...
                <div
                  className={`p-1 rounded-full ${
                    isRemoteVideoEnabled ? 'bg-green-500' : 'bg-red-500'
                  }`}
                >
                  {isRemoteVideoEnabled ? (
                    <Video className="w-3 h-3 text-white" />
